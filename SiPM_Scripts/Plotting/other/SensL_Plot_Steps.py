...
import matplotlib
#matplotlib.use('Agg') #remote plotting
import sys
...
#        plt.plot((np.amin(self.time),np.amax(self.time)), (np.mean(filtered_signal_2),np.mean(filtered_signal_2)), 'b--',label='MeanPeakLess') #rmsn from RemovePeaksBasedOnRMS3 = 2
    ...
    def Plot(self,pulse_h,pulse_h_c,pulse_a,pulse_a_c,ppc):
        ...
        ax2.set_yscale("log")
        ax2.set_xlabel('Pulse Area Integral [V bins]')
        ax2.set_ylabel('log$_{10}$ (Events) [#]')
        ax2.set_xlim(-0.01,0.12)
        ax2.hist(pulse_a,bins = nbins,color='blue',label='Area',histtype='step',log=True)
        ...
        ax3.plot(self.time,rawdat,'blue',label='-5$^\circ$C and V$_b$ 26.0V')
        ax3.legend()
        figure = plt.gcf() # get current figure
        figure.set_size_inches(7, 7)
