...
import sys
sys.path.append('/home/gebhardt/SiPM_MPIK/scripts')
import LecroyBinary
import numpy as np
...
class WfRed(object):
    ...
    def FindPeaks(self,filtered_signal_2,ite,rmsn):
        ...
        pulse_pos = detect_peaks(filtered_signal_2, rmsn, mpd=30,show=False)
    ...
    def Reduce_Data(self,rmsval):
        ...
        ax1.set_title('Integration Window')
        ax1.set_xlabel('Time [$\mu$s]')
        ax1.set_ylabel('Voltage [V]')
        ax2.set_xlabel('Time [$\mu$s]')
        ax1.set_xlim(45,46)
        ax2.set_xlim(45.4,45.6)
        ...
        x1=45.5164-(7*0.4)/1000
        x2=45.5164+(10*0.4)/1000
        y1=0.005
        y2=0.005
        ax2.set_title('Zoom')
...
def GetName(destT,destV,Date):
    ...
    path = '/home/gebhardt/SiPM_MPIK/scripts/Data/Date'+str(Date)+'/'+str(destT)+'deg/'+str(destV)+'V/'
    name = 'HAM_T' + str(destT) + '_Vb' + str(destV) + '.trc'
    compl_name = os.path.join(path,name)
    return compl_name

def GetSavePath(Date):
    savepath = '/home/gebhardt/SiPM_MPIK/scripts/Data/Date'+str(Date)+'/'
    return savepath
