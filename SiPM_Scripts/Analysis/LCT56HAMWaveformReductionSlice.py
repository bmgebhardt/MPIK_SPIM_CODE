...
fromregr = False
savestr = 'SciPYPFIntWin22'#'FixedGuess2mV22IntWinMPD10'# 1 'FixedGuess' 2 'Gainx0p25' 3 'FixedVal'
...
class WfRed(object):
    ...
    def Slownoise_Smooth(self,data,ite):
        ...
        #window = sig.general_gaussian(win_smooth_ns*samples_per_ns, p=1.0, sig=sig_smooth_ns*samples_per_ns)#sig.general_gaussian(nsamples, p=1.0, sig=nsamples/5)  #changed window to check influence of smoothing on integration window
        ...
        '''
        self.filtered_signal_1 = data_at_0
    ...
    def FindPeaks(self,filtered_signal_2):
        ...
        #print rms
        if args.plot == 3:
            pulse_pos = detect_peaks(filtered_signal_2, rms, mpd=10,show=True)
        else:
            pulse_pos = detect_peaks(filtered_signal_2, rms, mpd=10,show=False)
    ...
    def FindPeaks2(self,data):
        """
        new Peak finding method based on scipy signal
        raw data as input, no previous smoothing needed
        Variables:
        https://docs.scipy.org/doc/scipy-0.14.0/reference/generated/scipy.signal.find_peaks_cwt.html
        """

        width_ns = np.arange(10.,12.)
        samples_per_ns = 2.5

        pulse_pos = sig.find_peaks_cwt(data,widths=width_ns*samples_per_ns,noise_perc=10)
        return pulse_pos
    ...
    def Pulse(self,data,pulse_pos):
        ...
        wl = 2#2 # in ns, so 2.5 samples == 1 ns, 10 ns = 25 samples
        wr = 2#4 
        ...
        '''
        wl = 10#2 # in ns, so 2.5 samples == 1 ns, 10 ns = 25 samples
        wr = 10#4 
        for i, pos in enumerate(pulse_pos):
            pulse_area = []
            start_bin =int(pos - (wl * (self.samplerate / 1e9)))
            ...
            self.pulse_area.append(0)
            for j in range(start_bin, end_bin):
                pulse_area.append(self.filtered_signal_1[j]) #create list of lists?
                self.pulse_area[-1] += self.filtered_signal_1[j]

            x=(wl * (self.samplerate / 1e9))+(wr * (self.samplerate / 1e9)) 
            print x
            print len(pulse_area)

            plt.figure(33)
            plt.ylabel('Signal [mV]')
            plt.xlabel('T [ns]')
            plt.plot(np.arange(0,x)/2.5,pulse_area)
            plt.show()
        ''' 
    ...
    def Plot(self,pulse_h,pulse_h_c,pulse_a,pulse_a_c,ppc):
        ...
        ax1.hist(pulse_h,bins = nbins/10,color='grey',label='Height',histtype='step')
        #ax1.hist(pulse_h_c,bins = nbins/4,color='blue',label='Height Clean',histtype='step')
        ax1.hist(self.peakless,bins = nbins/50,color='green',histtype='step')
        plt.legend()
    ...
    def GetZeroPeak(self):#still needed, zeropeak now from filtered2
        ...
        self.meanzero.append((self.peakless))
    ...
    def Reduce_Data(self):
        ...
        for ite in range(0,calcseg):    
            ...
            '''
            pulse_pos = self.FindPeaks2(data)
            plt.figure()
            plt.plot(time,data,c='lightgrey')
            plt.scatter(time[pulse_pos],data[pulse_pos],c='k')
            plt.show()
            '''
...
def main():
    ...
    if args.date:
        ...
        datE = '0510162-25'#'0411161'          
        print datE
    Date, T, Vb = LoadRunCard(datE)
    high_pass = 0.002 #peak find threshold -r2
